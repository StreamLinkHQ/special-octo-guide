...
const AddonResponseViewer: React.FC<AddonResponseViewerProps> = ({
  ...
  const renderContent = () => {
    switch (agenda.action) {
      case "Quiz":
        // Pass agendaId and isLive flag to leverage SDK optimizations
        return <QuizLeaderboard agendaId={agenda.id} isLive={false} />;

      case "Poll":
        // Pass agendaId and isLive flag to leverage SDK optimizations
        return <PollResults agendaId={agenda.id} isLive={false} />;
