...
const InStreamWalletModal = ({ isOpen, onClose }: InStreamWalletModalProps) => {
  ...
  const formatTransaction = useCallback((tx: any, isStreamSpecific = false) => {
  ...
  const formattedMyTransactions = useMemo(() => {
    ...
    return myStreamTransactions.map((tx) => formatTransaction(tx, true)).sort((a, b) => 
  ...
  const formattedStreamTransactions = useMemo(() => {
    ...
    return allStreamTransactions.map((tx) => formatTransaction(tx, true)).sort((a, b) => 
      new Date(b.date).getTime() - new Date(a.date).getTime()
    );
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = `stream_transactions_${new Date().toISOString().split('T')[0]}.csv`;
        ...
        <div className="bg-purple-600 p-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-white font-bold text-lg">
              {currentView === 'selectRecipient' ? 'Select Recipient' : 'My Wallet'}
            </h3>
          </div>
          <button
            onClick={onClose}
              ...
              <div className="p-4 space-y-4">
                {activeTab === 'balance' && (
                  <>
                    ...
                    <div className="bg-purple-600 rounded-xl p-5 text-white shadow-lg">
                      ...
                      <div className="text-3xl font-bold mb-4">${balance.toFixed(6)}</div>
                    ...
                    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
                      ...
                      <div className="text-sm font-mono text-gray-800 break-all">
                        {wallet?.address ? `${wallet.address.slice(0, 8)}...${wallet.address.slice(-8)}` : "Not connected"}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                        <div className="text-xs text-gray-500 mb-1">Stream Earnings</div>
                        <div className="text-lg font-bold text-purple-600">${calculateMyStats.totalEarnings.toFixed(2)}</div>
                      </div>
                      <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                        <div className="text-xs text-gray-500 mb-1">Pending</div>
                        <div className="text-lg font-bold text-yellow-600">${calculateMyStats.pendingAmount.toFixed(2)}</div>
                      </div>
                    </div>
                    ...
                    <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-200">
                      ...
                      {myStreamLoading ? (
                        ...
                        <div className="space-y-2">
                          {formattedMyTransactions.slice(0, 5).map((tx: any) => (
                ...
                {activeTab === 'myTransactions' && (
                  <>
                    {/* Header with actions */}
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-gray-900">My Stream Transactions</h4>
                      <div className="flex gap-2">
                        <button
                          onClick={handleRefresh}
                          ...
                          title="Refresh"
                        >
                          <RefreshCw className={`w-4 h-4 text-gray-600 ${myStreamLoading ? 'animate-spin' : ''}`} />
                        </button>
                        <button
                    ...
                    <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          ...
                          <div className="text-xl font-bold text-purple-600">${calculateMyStats.totalEarnings.toFixed(2)}</div>
                        </div>
                        <div>
                          <div className="text-xs text-gray-500 mb-1">Transactions</div>
                          <div className="text-xl font-bold text-gray-900">{calculateMyStats.transactionCount}</div>
                        </div>
                      </div>
                    </div>
                    ...
                    {myStreamLoading ? (
                      ...
                      <div className="space-y-3">
                        {formattedMyTransactions.map((tx: any) => (
                          <div key={tx.id} className="bg-white rounded-lg p-4 shadow-sm border border-gray-200 hover:border-purple-300 transition-colors">
                            <div className="flex items-start justify-between">
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-gray-500 mt-0.5">{new Date(tx.date).toLocaleDateString()}</div>
                                </div>
                              </div>
                      ...
                      <div className="bg-white rounded-lg p-8 shadow-sm border border-gray-200 text-center">
                        ...
                        <p className="text-gray-500">No transactions in this stream yet</p>
                      </div>
                    )}
                  </>
                )}
                ...
                {activeTab === 'streamTransactions' && (
                  <>
                    {/* Header with actions */}
                    <div className="flex items-center justify-between mb-2">
                      <h4 className="font-semibold text-gray-900">All Stream Activity</h4>
                      <div className="flex gap-2">
                        <button
                          onClick={handleRefresh}
                          ...
                          title="Refresh"
                        >
                          <RefreshCw className={`w-4 h-4 text-gray-600 ${streamLoading ? 'animate-spin' : ''}`} />
                        </button>
                        <button
                          onClick={handleExport}
                      ...
                      <div className="bg-white rounded-lg p-4 shadow-sm border border-gray-200">
                        <div className="grid grid-cols-2 gap-4">
                          <div>
                            ...
                            <div className="text-xl font-bold text-purple-600">${parseFloat(streamStats.totalVolume || '0').toFixed(2)}</div>
                          </div>
                          <div>
                            <div className="text-xs text-gray-500 mb-1">Total Transactions</div>
                            <div className="text-xl font-bold text-gray-900">{streamStats.totalTransactions || 0}</div>
                          </div>
                        </div>
                      </div>
                    )}
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  ...
                                  <div className="text-xs text-gray-500 mt-0.5">
                                    ...
                                    {tx.user?.name && (
                                      <span className="ml-2">• {tx.user.name}</span>
                                    )}
                                  </div>
                                </div>
                              </div>
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50 p-4">
            <div className="mb-4">
              ...
              <p className="text-xs text-gray-500">
                {availableRecipients.length} {availableRecipients.length === 1 ? 'participant' : 'participants'} available
              </p>
            </div>
            ...
            {availableRecipients.length > 0 ? (
              ...
              <div className="bg-white rounded-lg p-8 text-center">
                ...
                <p className="text-xs text-gray-400">Other participants need to join the call first</p>
              </div>
            )}
          </div>
        )}
