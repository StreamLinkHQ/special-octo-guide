...
interface ContestConfigContextType {
  ...
  setPanelOpen: (open: boolean) => void;
  contestReady: boolean;
  setContestReady: (ready: boolean) => void;
}
  ...
  const getDefaultConfig = (type: ContestType): ContestConfig => {
    ...
    if (type === "turn-based") {
      return {
        ...
        type: "turn-based",
        plugin: {
          turnDuration: 120, // Default to 120s
          autoAdvance: false, // Default to manual control
          votingMode: 'final-only', // Default voting mode
          turnVotingDuration: 30, // Default turn voting duration
        },
      };
    }
  ...
  const broadcastConfig = (
    ...
    if (websocket && isHost && roomName) {
      ...
      websocket.sendMessage("updateContestConfig", {
        config: newConfig,
        contestType: newType,
        contestReady: ready !== undefined ? ready : contestReady,
        roomName: roomName,
      });
    }
  };
